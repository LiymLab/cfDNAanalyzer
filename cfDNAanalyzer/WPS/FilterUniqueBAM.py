  ...
  if options.consensus:
    ...
    if len(reads) > 1:
      ...
      for pos in range(len(reads[0].seq)):
        ...
        if count > 0:
          total_prob = math.log10(max(0.000000001,sum([10**x for x in bases])))
...
def get_consensus(reads):
  ...
  to_sort = [(x_y[1], -len(str(x_y[0])), x_y[0]) for x_y in list(cigar_count.items())]
  to_sort.sort()
...
for filename in files:
  ...
  for read in infile:
    ...
    if sum(map(len,iter(variants.values()))) > options.rbuffer and ((lcheck[0] != curpos[0]) or (lcheck[1]+options.MaxLength < curpos[1])):
      ...
      for cRG in list(variants.keys()):
        hvariants = {}
        for (hchr,outpos,outpos_r2),reads in variants[cRG].items():
  ...
  for RG in list(variants.keys()):
    for key,value in variants[RG].items():
