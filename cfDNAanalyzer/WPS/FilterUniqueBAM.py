...
"""
import logging
logging.getLogger('pysam').setLevel(logging.CRITICAL)
import sys
import os
import math
import pysam
  ...
  if options.consensus:
    ...
    if len(reads) > 1:
      ...
      for pos in range(len(reads[0].seq)):
        ...
        if count > 0:
          total_prob = math.log10(max(0.000000001,sum([10**x for x in bases])))  #diff
...
def get_consensus(reads):
  ...
  to_sort = [(x_y[1], -len(str(x_y[0])), x_y[0]) for x_y in list(cigar_count.items())] #diff
  to_sort.sort()
...
for filename in files:
  ...
  for read in infile:
    ...
    if sum(map(len,iter(variants.values()))) > options.rbuffer and ((lcheck[0] != curpos[0]) or (lcheck[1]+options.MaxLength < curpos[1])):
      lcheck = (curpos[0],curpos[1])
      if options.verbose: sys.stderr.write("Full buffer (%d)"%sum(map(len,iter(variants.values())))+str(curpos)+" \n")
      for cRG in list(variants.keys()):  #diff
        hvariants = {}
        for (hchr,outpos,outpos_r2),reads in variants[cRG].items(): #diff
    ...
    if read.is_paired: # PE DATA
      ...
      if (curpos != None) and ((read.tid,read.pos) != curpos):
        if options.fivePrime and (read.tid == curpos[0]):
          ...
          hvariants = {}
          for key,value in curvariants[RG].items(): #diff
            if (key[1] < hpos):
          ...
          for key,value in curvariants[RG].items():
            cread = get_consensus_SR(value[0])
            if cread == None: 
              #if options.verbose: sys.stderr.write("FAILED CONSENSUS\n")
              continue
  ...
  for RG in list(curvariants.keys()):
    for key,value in curvariants[RG].items():
  ...
  for RG in list(variants.keys()): #diff
    for key,value in variants[RG].items(): #diff
