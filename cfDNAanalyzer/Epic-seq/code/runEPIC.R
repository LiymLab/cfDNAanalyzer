...
runepic <- function(inputlist)
{
  ...
  if (is.null(inputlist$tssinfo))
  {
    ...
    tssinfo = paste0(epic_dir,"/priordata/all.tss.genes.canonical.ensembl75.txt")
    # end
  }else{
  ...
  {
    # 20240711 zjp modified
    # panelbed = paste0("./selectors_windows/all.tss.genes.canonical.ensembl75.selector.txt")
  ...
  epiccommand = paste(paste0(epic_dir,"/epic_wrapper.sh "),bamdir,tssinfo,outdir,panelbed,targeted,groupsize,
                      groupref,mapq,skipHist,skipOCFFrags,0,0,0,epic_dir,sep=" ")
  # end
