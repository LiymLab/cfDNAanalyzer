...
option_list <- list(
  ...
  # 20241027 Zhou Junpeng modified
...
if (genomeBuild == "hg19") {
  chrom_info <- read.table(paste0(script_dir,"/chrom_info/chromInfo_hg19.txt"), header = FALSE)
  seqinfo <- Seqinfo(seqnames = chrom_info$V1, seqlengths = chrom_info$V2, genome = "hg19")
} else if (genomeBuild == "hg38") {
  chrom_info <- read.table(paste0(script_dir,"/chrom_info/chromInfo_hg38.txt"), header = FALSE)
